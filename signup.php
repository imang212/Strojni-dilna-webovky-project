...
if ($conn -> connect_error) {
	header("Location: ../Strojni%20dilna%20webovky/register.php?register=connect");
  echo "Failed to connect to MySQL: " . $mysqli -> connect_error;
  exit();
}
elseif($pass1 != $pass2){
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=passdifferent");
  exit();
}
elseif(strlen($_POST["password_1"]) <= '6' || strlen($_POST["password_1"]) >= '20') {
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=passlength");
  exit();      
}
elseif(isset($checkbox) != 'ano'){
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=podminky");
  exit();
}
...
|| empty($_POST["password_2"]) || empty($_POST["phonenumber"]) || empty($_POST["datum_narozeni"])) {
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=empty");
  exit();
} 
elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)) { 
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=emailincorrect");
  exit();
}
elseif (!preg_match("/^[a-zA-Z ]*$/",$name)) {
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=nameincorrect");
  exit();
}
elseif (!preg_match("/^[a-zA-Z ]*$/",$lastname)) {
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=surnameincorrect");
  exit();
}
...
elseif($specialpismena) {
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=usernameincorrect");
  exit();
}
else if(mysqli_num_rows($result_name) == 1){
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=usernameexist");
  exit();
}

else if(mysqli_num_rows($result_email) == 1){
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=emailexist");
  exit();
}
elseif (!preg_match('/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{6,20}$/', $pass1)) {
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=passincorrect");
  exit();
}
elseif(empty($checkbox)){
  ...
  header("Location: ../Strojni%20dilna%20webovky/register.php?register=success");
  sleep(3);
  header("Location: ../Strojni%20dilna%20webovky/login.php?login=registered");
  exit();
}
?>
